<?php

$database = array(
  "membres" => array(
    "email" => "membres@wsf.fr",
    "password" => "5fa9c3cbc25addd13cc516171ea722ce"
  ),
  "admin" => array(
    "email" => "admin@wsf.fr",
    "password" => "21232f297a57a5a743894a0e4a801fc3"
  ),
  "series" => array(
    array(
      "Nom" => "Forrest Gump",
      "Annee" => 1994,
      "Durée" => "2h20min",
      "Réalisateur" => "Robert Zemeckis",
      "Acteurs" => array("Tom Hanks", "Gary Siniseplus"),
      "Genres" => array("Comédie dramatique" , "Romance"),
      "Like" => 100806,
      "Poster" => "http://www.goldposter.com/wp-content/uploads/2015/04/Forrest-Gump_poster_goldposter_com_7.jpg"
    ),
   ), 
  "movies" => array(
    array(
      "Nom" => "Forrest Gump",
      "Annee" => 1994,
      "Durée" => "2h20min",
      "Réalisateur" => "Robert Zemeckis",
      "Acteurs" => array("Tom Hanks", "Gary Siniseplus"),
      "Genres" => array("Comédie dramatique" , "Romance"),
      "Like" => 100806,
      "Poster" => "http://www.goldposter.com/wp-content/uploads/2015/04/Forrest-Gump_poster_goldposter_com_7.jpg"
    ),
    array(
      "Nom" => "Django Unchained",
      "Annee" => 2013,
      "Durée" => "2h44min",
      "Réalisateur" => "Quentin Tarantino",
      "Acteurs" => array("Jamie Foxx", "Christoph Waltz"),
      "Genres" => array("Western"),
      "Like" => 10030,
      "Poster" => "http://www.filmosphere.com/wp-content/uploads/2012/12/Django-Unchained-affiche.jpg"
    ),
    array(
      "Nom" => "La Ligne verte",
      "Annee" => 2000,
      "Durée" => "3h9min",
      "Réalisateur" => "Frank Darabont",
      "Acteurs" => array("Tom Hanks", "Michael Clarke Duncan"),
      "Genres" => array("Fantastique" , "Policier"),
      "Like" => 20030,
      "Poster" => "http://www.notrecinema.com/images/cache/la-ligne-verte-poster_50432_40929.jpg",
    ),
    array(
      "Nom" => "Gran Torino",
      "Annee" => 2009,
      "Durée" => "1h51min",
      "Réalisateur" => "Clint Eastwood",
      "Acteurs" => array("Clint Eastwood", "Bee Vangplus"),
      "Genres" => array("Drame" , "Thriller"),
      "Like" => 1030,
      "Poster" => "http://images.moviepostershop.com/gran-torino-movie-poster-2008-1020420150.jpg",
    ),
    array(
      "Nom" => "La Liste de Schindler",
      "Annee" => 1994,
      "Durée" => "3h15min",
      "Réalisateur" => "Steven Spielberg",
      "Acteurs" => array("Liam Neeson", "Ben Kingsleyplus"),
      "Genres" => array("Historique", "Drame"),
      "Like" => 60070,
      "Poster" => "http://www.incine.fr/media/affiches_film/big/la-liste-de-schindler-1373363089-60.jpg",
    ),
    array(
      "Nom" => "The Dark Knight, Le Chevalier Noir",
      "Annee" => 2008,
      "Durée" => "2h27min",
      "Réalisateur" => "Christopher Nolan",
      "Acteurs" => array("Christian Bale", "Heath Ledger"),
      "Genres" => array("Action" , "Drame"),
      "Like" => 89030,
      "Poster" => "http://fr.web.img2.acsta.net/medias/nmedia/18/63/97/89/18949761.jpg",
    ),
    array(
      "Nom" => "Le Parrain",
      "Annee" => 1972,
      "Durée" => "2h55min",
      "Réalisateur" => "Francis Ford Coppola",
      "Acteurs" => array("Marlon Brando", "Al Pacino"),
      "Genres" => array("Policier" , "Drame"),
      "Like" => 60180,
      "Poster" => "http://fr.web.img4.acsta.net/medias/nmedia/18/35/57/73/18660716.jpg",
    ),
    array(
      "Nom" => "Pulp Fiction",
      "Annee" => 1994,
      "Durée" => "2h29min",
      "Réalisateur" => "Quentin Tarantino",
      "Acteurs" => array("John Travolta", "Samuel L. Jackson"),
      "Genres" => array("Policier" , "Thriller"),
      "Like" => 40078,
      "Poster" => "http://www.movpins.com/big/MV5BMTU2Mjc0MTg4MF5BMl5BanBnXkFtZTcwOTA0MzU5Ng/pulp-fiction-(1994)-large-picture.jpg",
    ),
    array(
      "Nom" => "Le Seigneur des anneaux",
      "Annee" => 2003,
      "Durée" => "3h20min",
      "Réalisateur" => "Peter Jackson",
      "Acteurs" => array("Elijah Wood", "Sean Astin"),
      "Genres" => array("Fantastique" , "Aventure"),
      "Like" => 80050,
      "Poster" => "http://papystreaming.com/fr/files/2013/09/Le-Seigneur-des-anneaux-Le-Retour-du-roi.jpg",
    ),
    array(
      "Nom" => "Fight Club",
      "Annee" => 1999,
      "Durée" => "2h15min",
      "Réalisateur" => "David Fincher",
      "Acteurs" => array("Brad Pitt", "Edward Norton"),
      "Genres" => array("Thriller" , "Drame"),
      "Like" => 8010,
      "Poster" => "https://ok2disconnectportfolio.files.wordpress.com/2012/02/fight-club-hi-res-poster-vertical-a31.jpg",
    ),
    array(
      "Nom" => "Intouchables",
      "Annee" => 2011,
      "Durée" => "1h52min",
      "Réalisateur" => "Eric Toledano",
      "Acteurs" => array("François Cluzet", "Omar Sy"),
      "Genres" => array("Comédie"),
      "Like" => 89010,
      "Poster" => "http://ourfaithinaction.net/wp-content/uploads/2012/07/the-intouchables-movie-poster1.jpg",
    ),
    array(
      "Nom" => "Mommy",
      "Annee" => 2014,
      "Durée" => "2h18min",
      "Réalisateur" => "Xavier Dolan",
      "Acteurs" => array("Viviane Pascal", "Antoine-Olivier "),
      "Genres" => array("Drame"),
      "Like" => 59070,
      "Poster" => "https://41.media.tumblr.com/64a0d6d20e5e419b4b894aef542a7abb/tumblr_nm5m3etarH1szjhaso1_500.png",
    )
  ),
  "series" => array(
    array(
      "Nom" => "Secrets And Lies (US)",
      "Annee" => 2015,
      "Acteurs" => array("Juliette Lewis", "Michael Ealy"),
      "Genres" => array("Drame"),
      "Like" => 114,
      "Poster" => "http://data1.ibtimes.co.in/en/full/571097/secrets-lies-spoilers.jpg"
    ),
    array(
      "Nom" => "The Walking Dead",
      "Annee" => 2010,
      "Acteurs" => array("Andrew Lincoln", "Steven Yeun"),
      "Genres" => array("Drame" , "Epouvante-horreur"),
      "Like" => 25708,
      "Poster" => "http://spinoff.comicbookresources.com/wp-content/uploads/2012/09/walking-dead-s3-poster2.jpg"
    ),
    array(
      "Nom" => "Game of Thrones",
      "Annee" => 2011,
      "Acteurs" => array("Peter Dinklage", "Nikolaj Coster-Waldau"),
      "Genres" => array("Drame" , "Fantastique"),
      "Like" => 31435,
      "Poster" => "https://www.movieposter.com/posters/archive/main/117/MPW-58919"
    ),
    array(
      "Nom" => "Arrow",
      "Annee" => 2012,
      "Acteurs" => array("Stephen Amell", "Katie Cassidy"),
      "Genres" => array("Drame" , "Fantastique"),
      "Like" => 12087,
      "Poster" => "http://smhttp.42308.nexcesscdn.net/8016BCA/greenarrow/wp-content/gallery/arrow-posters/novembersweepspromoart.jpg"
    ),
    array(
      "Nom" => "Narcos",
      "Annee" => 2015,
      "Acteurs" => array("Wagner Moura", "Pedro Pascal"),
      "Genres" => array("Drame" , "Policier"),
      "Like" => 5181,
      "Poster" => "http://cdn2-www.comingsoon.net/assets/uploads/gallery/narcos/narcos0002.jpg"
    ),
    array(
      "Nom" => "Les 100",
      "Annee" => 2014,
      "Acteurs" => array("Eliza Taylor", "Paige Turco"),
      "Genres" => array("Aventure" , "Drame"),
      "Like" => 5499,
      "Poster" => "http://www.cinemay.com/wp-content/uploads/2014/11/The-100-New-Promotional-Poster2.png"
    ),

    array(
      "Nom" => "Mr. Robot",
      "Annee" => 2014,
      "Acteurs" => array("Rami Malek", "Carly Chaikin"),
      "Genres" => array("Drame" , "Thriller"),
      "Like" => 1086,
      "Poster" => "https://posterspy.com/wp-content/uploads/2015/09/mr-robot-oficial.jpg"
    ),
    array(
      "Nom" => "Le Passager",
      "Annee" => 2014,
      "Acteurs" => array("Jean-Hugues Anglade", "Raphaëlle Agogué"),
      "Genres" => array("Thriller"),
      "Like" => 11,
      "Poster" => "http://cdn.cinemur.fr/posts/cache/Birdman.jpg"
    ),

    array(
      "Nom" => "Versailles",
      "Annee" => 2014,
      "Acteurs" => array("George Blagden", "Alexander Vlahos"),
      "Genres" => array("Drame" , "Historique"),
      "Like" => 59,
      "Poster" => "http://images.moviepostershop.com/versailles-movie-poster-2008-1020512155.jpg"
    ),
    array(
      "Nom" => "Profilage",
      "Annee" => 2014,
      "Acteurs" => array("Odile Vuillemin", "Philippe Bas"),
      "Genres" => array("Policier"),
      "Like" => 801,
      "Poster" => "http://www.notrecinema.com/images/cache/profilage-poster_468025_688.jpg"
    ),
    array(
      "Nom" => "Panthers / The Last Panthers",
      "Annee" => 2015,
      "Acteurs" => array("Samantha Morton", "Tahar Rahim"),
      "Genres" => array("Thriller"),
      "Like" => 28,
      "Poster" => "http://www.flickeringmyth.com/wp-content/uploads/2015/10/the-last-panthers-poster-600x890-600x890.jpg"
    ),
    array(
      "Nom" => "Vikings",
      "Annee" => 2013,
      "Acteurs" => array("Travis Fimmel", "Katheryn Winnick"),
      "Genres" => array("Aventure", "Drame"),
      "Like" => 63745,
      "Poster" => "http://images6.fanpop.com/image/photos/36800000/Vikings-tv-series-image-vikings-tv-series-36808484-1036-1500.jpg"
    ),
  )
);